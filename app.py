...
with tab_msg:
    ...
    if st.button('Enviar mensaje por Canvas', type='primary', use_container_width=True):
        if not confirm_send:
            ...
            try:
                response = client.send_conversation(ids, subject_canvas, msg, course_id=course_id, group_conversation=group_msg, mode='async' if len(ids) > 1 and not group_msg else 'sync')
                st.success(f'Mensaje enviado/encolado correctamente a {len(ids)} destinatario(s).')
                st.json(response if response not in (None, []) else {'estado': 'Canvas aceptó el envío asincrónico', 'destinatarios': len(ids)})
            except Exception as e:
